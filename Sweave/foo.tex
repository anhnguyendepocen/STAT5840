...
where the \texttt{include=FALSE} indicates that the output (text and graphics)
should not go here (they will be some place else) and the \texttt{label=fig1plot}
...
In this code chunk the \texttt{fig=TRUE} indicates that the chunk
generates a figure.  \texttt{Sweave} automagically makes both EPS and PDF
files for the figure and automagically generates an
appropriate \LaTeX\ \texttt{\includegraphics} command
