...
\begin{itemize}
\item The Bayesian model is  \(X_{1},X_{2},\ldots, X_{n}\sim f(x|\theta)\), where $f$ is the \emph{sampling distribution}.
\item We have \(\theta \sim g(\theta) \) where $g$ is the \emph{prior}.
\item Observe data \(x_{1},x_{2},\ldots,x_{n}\).
\item Base inferences on the posterior distribution
  \[
...
\textbf{Example:}  Estimating a normal mean with a Cauchy prior (Robert/Casella).
...
See the HANDOUT, \texttt{cauchyprior.pdf}
