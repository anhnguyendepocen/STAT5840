...
- Now, the James-Stein estimator depends on $0 < a < 2(p-2)$.

- Want to investigate three (3) things:
  - Show that the risk of J-S truly is less than $p$ for all $\theta$
  - See if we can speed up our estimates by introducing correlation (namely, by reusing simultations)
  - See what happens over a range of values of $a$


*Monte Carlo Experiment*

Naive way: 
1. Choose a grid of $\theta$ values (say, $0 < \theta < 10$).
2. For each $\theta$,
   1. Simulate 2000 values of $X|\theta = (X_{1},\ldots,X_{p})$ where $X_{i} \sim N(\theta,1)$, independent.
   2. Compute JSE, $\delta_{a}^{\mathrm{JS}}$
   3. Compute loss, $\sum \left( \delta_{i}^{\mathrm{JS}}(x) - \theta \right)^{2}$
   4. Average losses to get risk

Better way:
- use the same generated $x$'s for each calculated value of risk.  Change only $\theta$ from run to run.
...
Define an /antithetic pair/
\[
\frac{h(X_{i}) + h(Y_{i})}{2}
\]
then the /antithetic estimator/
\[
...
1. Using inversion
   ...
   - $U$ and $1-U$ are negatively correlated
   - $X = F^{-1}(U)$ and $Y = F^{-1}(1 - U)$

   *Example*  want to compute
   \[
   \int_{0}^{\infty}x^{2}\me^{-x} \diff x
   \]
   1. Boring way
      - Generate  \(X_{1},X_{2},\ldots,X_{m}\sim \mathrm{Exp}(1)\)
      \[
      \hat{I}=\frac{1}{m} \sum_{i = 1}^{m}X_{i}^{2}
      \]
   2. Better way
      - Generate  \(U_{1},U_{2},\ldots,U_{m}\sim \mathrm{Unif}(0,1)\)
      - Let $X_{i} = -\ln(U_{i})$ and $Y_{i} = -\ln(1 - U_{i})$
      \[
      \hat{I}=\frac{1}{m} \sum_{i = 1}^{m}\frac{X_{i}^{2} + Y_{i}^{2}}{2}
      \]

2. Use symmetry
   - $X \sim f(x)$ which is symmetric about $\mu$
   - $X - \mu$ has the same distribution as $\mu - X$
   - So $X$ has the same distribution as $2\mu - X$ (remember $U$ and $1 - U$)
   *Example*  want to compute
   \[
   \int_{0}^{\infty}x^{3}\phi(x) \diff x
   \]
   1. Boring way
      - Generate  \(X_{1},X_{2},\ldots,X_{m}\sim \mathrm{N}(0,1)\)
      \[
...
2. The variance of the ECDF is
   ...
   \]
3. In the nonparametric IID setting the ECDF $F_{n}$ is a /minimal sufficient statistic/ for $F$.
...
Please see the PRINTOUT, =bootmed.pdf=.  

*Example.* Bootstrapping the Standard Error of the Median, 2nd try.  In this handout we do it the right way.  We set up a function of two areguments, and use the =boot= function in =R=. 
