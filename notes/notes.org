...
#+BABEL: :session *R* :results output raw
#+LaTeX_CLASS: scrbook
...
b <- c(3, 3, 2, 1, 0.5, 0.5)
for (i in 1:6){
  f <- function(x){dbeta(x, shape1 = a[i], shape2 = b[i])}
  curve(f, main = paste("alpha = ", a[i], ", beta = ", b[i], sep = ""))
}
par(mfrow = c(1,1))
#+end_src
#+latex: \end{center}
...
*** Requirements of a Uniform Pseudo-Random Generator
...
_Repeatability_: the ability to repeat the same stream of numbers at any time during the simulation. Useful for comparing simulations using different methods.
...
- Probability Integral Transformation: :: Suppose \(X\) has a continuous CDF $F_{X}(x)=\P(X \leq x)$. Define the inverse CDF $F_{X}^{-1}(y)$.  If $U\sim\mathrm{Unif}(0,1)$, then $F_{X}^{-1}(U)$ is distributed according to $F_{X}$.
...
- Note: ::We assumed above that the CDF was continuous, but we didn't need to... the statement is true in all generality. In the general case we must define 
  \[
...
**** Using Exponentials to build other distributions.

*Example.* Generating Gammas.
...
- Know: :: Fisher's $F$ distribution $F(\alpha,\beta)$.
  \[
...
*Example.* Generating $N(0,1)$s /exactly/.
...
GREAT! Now we can generate normals. It turns out that R has a built in algorithm for calculating Normals, which is called with the command =rnorm=.
...
*R:* The Box-Muller Algorithm to simulate two standard normals:
...
Thus it is EASY to simulate from a Cauchy, and further, we know already that Cauchy has /heavier tails/ than the Normal, suggesting that there is an $M$ such that Normal $\leq$ $M\cdot$ /Cauchy/.  That is, we should find $M$ that satisfies 
\begin{align*}
...
2. Compute $T$. 
3. Record if $|T|>t_{m+n-2}(\alpha/2)=1.7341$. (Here we set $\alpha=0.10$.) 
4. Repeat. 
...
*Example.* Cauchy tail probabilities. (Ripley) We would like to estimate the quantity 
\[
...
\]
Since the functional form is the same, we may complete another integration by parts and we will find that the variance of $\hat{p}_{4}$ is $\mbox{Var}(\hat{p}_{4})=0.000095525/m$ which is /an additional 99.6% reduction in variance/! If we compare the variance of $\hat{p}_{1}$ with that of $\hat{p}_{4}$
we find: 
\[
...
*Example.* Exponential and Lognormal Comparison (book). Here we wish to estimate the parameter $\lambda$ based on the observation of a random variable $X$, where $X$ is distributed from two different populations. The first is exponential, $X\sim\mathrm{Exp}(\lambda)$ with PDF 
\[
